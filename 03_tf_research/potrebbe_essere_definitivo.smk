...
# ricordarsi che qui lo snakemake delle api utilizza una altra fonte di input rispetto a quella che sto utilizzando io (utilizzano il file prodotto dalla rule create_pep_fasta)
...
rule aggregate_tables:
	input:
		...
		disco_dir = "../02_orthology/02_DISCO_OG",
		map = "../00_dataset/00_GAGA_download/GAGA_vs_personal_ID.tsv"
	output:
	...
	run:
		...
		with open(input.map) as f:
			for line in f:
				...
				sample_to_species_short[species_full] = species_short  # es. Acafer -> Afer
		...
		def build_orthogroup_dict_from_faa(faa_dir, species_full):
			"""Costruisce {gene_id -> ortogruppo} leggendo i file .faa di DISCO"""
			result = defaultdict(lambda: 'NA')
			for faa_file in os.listdir(faa_dir):
				if not faa_file.endswith('.faa'):
					continue
				orthogroup = faa_file.replace('.faa', '')
				with open(os.path.join(faa_dir, faa_file)) as f:
					for line in f:
						if line.startswith('>'):
							header = line.strip().lstrip('>')
							if '|' not in header:
								continue
							sp, gene = header.split('|')
							if sp == species_full:
		...
		for sample, file in zip(SAMPLES, input.species_tables):
			print(sample)
			species_short = sample_to_species_short[sample]  # es. Afer

			db = build_orthogroup_dict_from_faa(input.disco_dir, species_full)
			if '' in db:
				del db['']
