...
rule aggregate_tables:
	...
	run:
		...
		for sample, file in zip(SAMPLES, input.species_tables):
			...
			# Se il sample non è presente nella mappa, i nomi delle proteine
			# vengono considerati già corretti e non vengono modificati
			if sample not in sample_to_species_short:
				print(f"WARNING: '{sample}' non trovato nella mappa ID. "
				      f"I nomi delle proteine di questa specie non verranno normalizzati.")
				species_short = None
				species_full = sample
			else:
