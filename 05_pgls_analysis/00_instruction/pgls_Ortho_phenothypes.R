...
# this script tiffers form pgls_phenotypes.R because of two main things: the MAD statistics is applied so that the outliers values are correctly managed, and it has been implemented the elimination of NAs that were present inside the index of the totalscore_MAXXXX.X.tsv files
...
process_motif_file <- function(file, tree, species_pheno, phenotype_col,
  ...
  #' this line allows to remove NAs lines from the index list
  data_raw <- data_raw[!is.na(data_raw$index), ]
