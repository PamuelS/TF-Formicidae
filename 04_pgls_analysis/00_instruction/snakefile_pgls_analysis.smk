...
rule all:
	input:
		expand("../01_pgls_results/{motif}/pgls5_castes.csv", motif=MOTIFS, phenotype=PHENOTYPES)
...
rule pgls_phenotypes:
	...
	shell:
		...
		"OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 Rscript pgls_phenothypes.R {input.tree} {input.phenotypes} {input.motifs} {params.cores} {output.pgls5}"
